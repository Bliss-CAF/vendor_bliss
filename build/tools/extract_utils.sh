...
function setup_vendor() {
    ...
    export PRODUCTMK="$BLISS_ROOT"/"$OUTDIR"/"$DEVICE"-vendor.mk
...
function oat2dex() {
    ...
    if grep "classes.dex" "$BLISS_TARGET" >/dev/null; then
...
function extract() {
    ...
    for (( i=1; i<COUNT+1; i++ )); do
        ...
        if [ "$SRC" = "adb" ]; then
            ...
            cp "$SRC/$FILE" "$DEST"
            # if file does not exist try BLISS target
            if [ "$?" != "0" ]; then
                cp "$SRC/$TARGET" "$DEST"
            fi
        fi
